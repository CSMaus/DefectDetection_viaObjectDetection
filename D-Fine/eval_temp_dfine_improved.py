# code for predictions with improved temporal model
import os
import json
import torch
...
# Import the improved TemporalDFine class
from temp_dfine_over_improved import TemporalDFine
...
def draw_boxes(image: Image.Image, boxes, labels, scores, id2label, threshold=0.3):
    ...
    # Draw boxes
    for (x1, y1, x2, y2), lbl, sc in zip(filtered_boxes, filtered_labels, filtered_scores):
        # Use different colors for different classes
        color = "red" if lbl < len(id2label) - 1 else "blue"  # Red for defects, blue for no_object
...
def inference_on_folder(input_folder, output_folder, checkpoint, seq_len=50, threshold=0.3):
    ...
    for i, chunk in enumerate(chunk_list(img_paths, seq_len)):
        ...
        with torch.no_grad():
            ...
            for i, sz in enumerate(sizes):
                # Create a SimpleNamespace object that mimics the expected structure
            ...
            for j, (path, result) in enumerate(zip(chunk, results)):
                img = imgs[j]
                ...
                img = draw_boxes(img, boxes, labels, scores, id2label, threshold)
                ...
                defect_count = sum(1 for label, score in zip(labels, scores) if label < len(id2label) - 1 and score >= threshold)
                print(f"  - Detected {defect_count} defects in {os.path.basename(path)}")
...
if __name__ == "__main__":
    ...
    OUTPUT_FOLDER = os.path.join("ds_manipulations/dataset/predictions_improved", FOLDER)
    if not os.path.exists(OUTPUT_FOLDER):
        os.makedirs(OUTPUT_FOLDER, exist_ok=True)
    
    # Path to your trained model checkpoint
    CKPT = "temporal_dfine_improved_epoch_0.pth"  # Use the last epoch from improved model
    SEQ_LEN = 50
    THRESHOLD = 0.4  # Lower threshold to catch more potential defects
    
    # Run inference
    inference_on_folder(INPUT_FOLDER, OUTPUT_FOLDER, CKPT, SEQ_LEN, THRESHOLD)
