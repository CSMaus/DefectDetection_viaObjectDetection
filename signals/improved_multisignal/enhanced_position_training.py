...
def train_enhanced_position_model(model, train_loader, val_loader, optimizer, scheduler, num_epochs, device, save_dir):
    ...
    for epoch in range(num_epochs):
        ...
        print(
            f"Train Pos Acc (IoU≥0.3): {train_pos_accuracy:.4f}, Train Pos Acc (IoU≥0.5): {train_pos_accuracy_strict:.4f}")
...
def plot_enhanced_training_history(history, save_path=None):
    ...
    stage_nums = [{'detection_only': 1, 'position_only': 2, 'joint_training': 3}[stage] for stage in
                  history['training_stage']]
