...
def plot_prediction_percentages(predictions, font_type='Arial', font_size=12):
    percentages = [pred * 100 for pred in predictions]
    plt.figure(figsize=(12, 6))
    plt.plot(percentages, marker='o', linestyle='-', color='b')
    # plt.title('Prediction Percentages for Signals', fontsize=font_size + 2, fontname=font_type)
    plt.xlabel('Signal Index', fontsize=font_size, fontname=font_type)
    plt.ylabel('Defective Probability (%)', fontsize=font_size, fontname=font_type)
...
predictions = load_and_predict('MultiSignalClassifier_model2.pth', signal_set_path, num_signals_per_set, signal_length)

plot_prediction_percentages(predictions, font_type='Times New Roman', font_size=12)
